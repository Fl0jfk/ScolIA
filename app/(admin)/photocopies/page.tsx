...
export default function PhotocopiesCouleurPage() {
  ...
  const [typeImpression, setTypeImpression] = useState<PhotoCopieTypeImpression>("COULEUR");
    ...
    () =>
      [...items]
        .filter(
          (i) =>
            i.status !== "EN_ATTENTE" &&
            isPhotoCopieDirectionCircuit(i) &&
            canManagePhotocopiesDemand(i, roles, establishments, user?.id),
        )
  ...
  const submit = async () => {
    ...
    if (!forOther && !userEmail) {
      setError(
        typeImpression === "COULEUR"
          ? "Votre compte ne comporte pas d'adresse e-mail : impossible de recevoir la décision de la direction."
          : "Votre compte ne comporte pas d'adresse e-mail : impossible de vous prévenir lorsque les photocopies seront prêtes.",
      );
      return;
    }
        ...
        <div className="min-w-0">
          <p className={`font-semibold ${dash.ink}`}>
            ...
            <span className="font-medium text-slate-500">
              {" "}
              · {photoCopieTypeImpressionLabel(normalizePhotoCopieTypeImpression(item.typeImpression))}
            </span>
          </p>
  ...
  return (
    <ModulePageShell maxWidthClass="max-w-[1500px]" tourModuleId="photocopies-couleur">
      <ModulePageHeader
        eyebrow="Services"
        title="Photocopies"
        description="Noir et blanc : envoi direct au service impressions. Couleur : validation de la direction avant impression."
      />
      ...
      {creator && !forOther && !userEmail && (
        <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-900 mb-6">
          Compte sans e-mail principal : vous ne pouvez pas déposer une demande tant qu&apos;aucune adresse n&apos;est
          joignable pour vous (suivi et retrait).
        </div>
      )}
      ...
      {readyMine.length > 0 && (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 mb-6">
          ...
          <span className="block text-xs text-emerald-800 mt-1">
            Vos photocopies ont été traitées par le service impressions.
          </span>
        </div>
      )}
        ...
        <div className="mb-6 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-950">
          <span className="font-bold">
            {opsPrintQueue.length === 1
              ...
              : `${opsPrintQueue.length} photocopies à imprimer`}
          </span>
        ...
        {creator ? (
          <ModuleCard data-tour="photocopies-new" className="xl:col-span-1 h-fit" bodyClassName="p-6">
            ...
            <div className="space-y-4">
              ...
              <fieldset>
                <legend className={`mb-2 block ${dash.fieldLabel}`}>Type d&apos;impression</legend>
                <div className="space-y-2">
                  <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 cursor-pointer has-[:checked]:border-slate-400 has-[:checked]:bg-slate-50">
                    <input
                      type="radio"
                      name="typeImpression"
                      className="mt-1 h-4 w-4 border-slate-300 text-indigo-600 focus:ring-indigo-500"
                      checked={typeImpression === "NOIR_BLANC"}
                      onChange={() => setTypeImpression("NOIR_BLANC")}
                    />
                    <span className="text-sm text-slate-700">
                      <span className="font-bold text-slate-900">Noir et blanc</span>
                      <span className="block text-xs text-slate-500 mt-0.5">
                        Envoi direct au service impressions — pas de validation direction.
                      </span>
                    </span>
                  </label>
                  <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 cursor-pointer has-[:checked]:border-amber-300 has-[:checked]:bg-amber-50/60">
                    <input
                      type="radio"
                      name="typeImpression"
                      className="mt-1 h-4 w-4 border-slate-300 text-indigo-600 focus:ring-indigo-500"
                      checked={typeImpression === "COULEUR"}
                      onChange={() => setTypeImpression("COULEUR")}
                    />
                    <span className="text-sm text-slate-700">
                      <span className="font-bold text-slate-900">Couleur</span>
                      <span className="block text-xs text-slate-500 mt-0.5">
                        La direction de l&apos;établissement accepte ou refuse avant impression.
                      </span>
                    </span>
                  </label>
                </div>
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  Jusqu&apos;à {PHOTOCOPIES_MAX_DOCUMENTS} PDF dans la même demande
                  {typeImpression === "COULEUR"
                    ? " — visibles par la direction puis par le service impressions."
                    : " — transmis directement au service impressions."}
                </p>
              </div>
              {error && (
                ...
                {saving
                  ...
                  : forOther
                    ...
                    : typeImpression === "COULEUR"
                      ...
                      : "Envoyer au service impressions"}
              </ModuleButton>
            </div>
          </ModuleCard>
        ) : (
          ...
          {isOpsHandler && (
            ...
            >
              ...
              <p className="text-xs text-slate-500 mb-4">
                Demandes acceptées (couleur) ou transmises directement (noir et blanc). Ouvrez chaque
                PDF, imprimez, puis marquez « imprimée / prête » pour prévenir le demandeur par e-mail.
              </p>
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">
                                  ...
                                  {" · "}
                                  {photoCopieTypeImpressionLabel(
                                    normalizePhotoCopieTypeImpression(item.typeImpression),
                                  )}
                                  {item.createdBy.email ? ` · ${item.createdBy.email}` : ""}
                                  {isPhotoCopieNoirBlanc(item)
                                    ? " · transmis "
                                    : " · acceptée "}
                                  {item.decidedAt
