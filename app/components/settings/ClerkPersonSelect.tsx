...
export function clerkMemberLabel(m: ClerkMemberOption): string {
...
export function findClerkMember(
  members: ClerkMemberOption[],
  opts: { id?: string; email?: string },
): ClerkMemberOption | undefined {
  const id = String(opts.id || "").trim();
  if (id) {
    const byId = members.find((m) => m.clerkUserId === id);
    if (byId) return byId;
  }
...
function MemberSearchList({
  ...
  onPick: (member: ClerkMemberOption) => void;
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return members.filter((m) => {
      const hay = `${clerkMemberLabel(m)} ${m.email} ${m.lastName ?? ""}`.toLowerCase();
    ...
    <div className="space-y-2">
      ...
      <div className="max-h-44 divide-y divide-white/60 overflow-y-auto rounded-2xl border border-white/70 bg-white/60 backdrop-blur-sm">
        {filtered.length === 0 ? (
          ...
          filtered.map((m) => {
            const active = selectedIds.has(m.clerkUserId);
            return (
              <button
                key={m.clerkUserId}
                type="button"
              ...
              >
                <span className={`font-semibold ${dash.ink}`}>{clerkMemberLabel(m)}</span>
...
export default function ClerkPersonSelect({
  ...
  const selected = findClerkMember(activeMembers, { id: selectedId, email: selectedEmail });
  ...
  return (
    <div className="space-y-2">
      {selected ? (
        <SelectedChip
          name={clerkMemberLabel(selected)}
      ...
      {hasSelection && !picking ? (
        ...
        <MemberSearchList
          ...
          selectedIds={new Set(selected ? [selected.clerkUserId] : [])}
          onPick={(member) => {
            onChange(member);
            setPicking(false);
          }}
...
export function ClerkPeopleSelect({
  members,
  selectedEmails,
  onChange,
  loading,
  ...
  const selectedIds = new Set(matched.map((m) => m.clerkUserId));
