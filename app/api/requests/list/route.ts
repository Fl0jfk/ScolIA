...
async function hasStaffBoardAccess() {
  return canAccessRequestsStaffBoardFromSession();
}
...
export async function GET(req: NextRequest) {
  ...
  const scope = scopeParam ?? ((await hasStaffBoardAccess()) ? "board" : "submitted");
  try {
    try {
    ...
    if (scope === "board" || scope === "all" || scope === "my_queue") {
      if (!(await hasStaffBoardAccess())) return new NextResponse("Accès refusé", { status: 403 });
